...
size_t sequential_image_downloader(int num_images)
{
    ...
    for (int i = 1; i <= num_images; i++)
    {
        ...
        printf("Image #%d downloaded, total bytes so far: %d\n", i, static_cast<unsigned int>(total_bytes));
    }
    return total_bytes;
}
...
size_t download_image(int image_num)
{
    ...
    snprintf(url, sizeof(url), "http://699340.youcanlearnit.net/image%03d.jpg", ((image_num % 50) + 1));
...
int main()
{
    ...
    constexpr int NUM_IMAGES = 50;

    printf("Evaluating Sequential Implementation...\n");
    std::chrono::duration<double> sequential_time(0);
    size_t sequential_result = sequential_image_downloader(NUM_IMAGES);
    ...
    size_t parallel_result = parallel_image_downloader(NUM_IMAGES);
    for (int i = 0; i < NUM_EVAL_RUNS; i++)
    {
        auto startTime = std::chrono::high_resolution_clock::now();
